...
namespace BookManagementSystemOrder.Repository
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public async Task<ResultModel<BookOrder>> GetBookOrders()
        {
            ...
            try
            {
                var orders = await _context.BookOrders.ToListAsync();
                if (orders != null && orders.Any())
                {
                    result.status = "00";
                    result.success = true;
        ...
        public async Task<ResultModel<string>> DeleteOrderById(int id)
        {
            ResultModel<string> result = new ResultModel<string>();
            try
            {
                var deleteModel = await _context.Orders.Where(x => x.OrderId == id).FirstOrDefaultAsync();
                if (deleteModel != null)
                    _context.Orders.Remove(deleteModel);
                var bookOrderModel = await _context.BookOrders.Where(x => x.OrderId == id).ToListAsync();
                if (bookOrderModel != null)
                    _context.BookOrders.RemoveRange(bookOrderModel);
